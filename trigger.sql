...
BEGIN
	...
	VALUES (NEW.MATCH_ID,(NEW.MATCH_ID-1)/380+1,NEW.MATCH_ID%380);
END;
...
BEGIN
	...
	where new.ASSIST_BY = PLAYER_ID and SEASON_ID = (NEW.MATCH_ID-1)/380 +1;
	UPDATE PLAYER_STATS 
...
BEGIN
	...
	CASE
		...
		ELSE POINTS
	END
	WHERE CLUB_STATS.SEASON_ID = (NEW.MATCH_ID -1)/380+1 AND (CLUB_STATS.CLUB_ID = (SELECT HOME_CLUB FROM ( SELECT * FROM MATCH WHERE MATCH_ID=NEW.MATCH_ID) NATURAL JOIN PLAYING_CLUBS) OR CLUB_STATS.CLUB_ID = (SELECT AWAY_CLUB FROM ( SELECT * FROM MATCH WHERE MATCH_ID=NEW.MATCH_ID) NATURAL JOIN PLAYING_CLUBS));
END;
...
SET STATUS = 
CASE 
	WHEN NEW.MATCH_ID = 380 THEN 'Finished'
	ELSE 'Ongoing'
END
WHERE SEASON_ID = (NEW.MATCH_ID-1)/380+1;
UPDATE PLAYER_STATS
...
where 	
		...
		)) and SEASON_ID = (new.MATCH_ID-1)/380 + 1; 
END;
...
where PLAYER_ID = new.PLAYER_ID and new.isSUB = 0 and SEASON_ID = (new.MATCH_ID-1)/380 + 1;
END;
...
UPDATE PLAYER_STATS
SET MATCHES_PLAYED = MATCHES_PLAYED + 1
where PLAYER_ID = new.PLAYER_ID and SEASON_ID = (new.MATCH_ID-1)/380 + 1;
END;
